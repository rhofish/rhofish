...
export class ParticleLifeSim {
  ...
  externalY = -1;
  externalStrength = 0; // 力的大小系数
  externalRadius = 0; // 影响半径（与 rMax 类似）
  // 辅助常量
  EPS = 1e-6;
  FORCE_BETA = 0.3;
  ...
  handleResize() {
    ...
    this.gridCols = Math.floor(this.worldWidth / this.cellSize);
    this.gridRows = Math.floor(this.worldHeight / this.cellSize);
    if (this.gridCols < 1)
        this.gridCols = 1;
    if (this.gridRows < 1)
        this.gridRows = 1;
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (this.externalRadius > this.EPS) {
        ...
        if (dist < this.externalRadius) {
          ...
          const factor = this.externalStrength * t;
...
window.addEventListener("dblclick", () => sim.randomizeMatrix());
// 缓存
let autoSave = localStorage.getItem("autoSave"); // 默认缓存
if (autoSave === null) {
  autoSave = true;
  localStorage.setItem("autoSave", autoSave);
}
if (autoSave) {
  window.addEventListener("load", () => sim.load());
