...
export class ParticleLifeSim {
  ...
  initSpeciesAppearance() {
    for (let i = 0; i < this.speciesCount; i++) {
      ...
      this.speciesRadii[i] = this.baseRadius + (i % 3);
    }
  }
  /**
  ...
  handleResize() {
    ...
    this.worldHeight = displayHeight;
    // 重建空间网格
    this.cellSize = this.interactionRadius;
    this.gridCols = Math.ceil(this.worldWidth / this.cellSize) + 1;
    this.gridRows = Math.ceil(this.worldHeight / this.cellSize) + 1;
  ...
  setExternalForce(x, y, radius, strength) {
    ...
    this.externalForce = Math.min(
      this.forceStrength,
      Math.max(-this.forceStrength, strength)
    );
  }
  /**
   * 清除外部力场
  ...
  draw() {
    ...
    this.ctx.fillText("RUOO.TOP", w - 15, h - 15); // 作者的私货，不准改！
  }
  /**
